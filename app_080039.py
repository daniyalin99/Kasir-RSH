...
if metode_input == "⌨️ Ketik Manual":
    ...
    st.info("Arahkan barcode fisik ke kamera laptop/HP. Sistem akan otomatis mendeteksi angkanya.")
...
if st.button("➕ Tambahkan ke Nota", use_container_width=True):
    if barcode_final and not df_master.empty:
        ...
        if not hasil_cari.empty:
            ...
            artikel = row['ARTIKEL'] if 'ARTIKEL' in row and pd.notna(row['ARTIKEL']) else "TIDAK ADA"
            desc = row['DESC'] if 'DESC' in row and pd.notna(row['DESC']) else "TIDAK ADA DESKRIPSI"
            harga_asli = int(row['PRICE_1']) if 'PRICE_1' in row and pd.notna(row['PRICE_1']) else 0
